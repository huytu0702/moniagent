...
class ExpenseExtractionTool(BaseModel):
    ...
    message_type: str = Field(default="text", description="Type of message: text or image")
...
class BudgetCheckTool(BaseModel):
    ...
    expense_amount: float = Field(description="Amount of the expense to check against budget")
...
class FinancialAdviceTool(BaseModel):
    ...
    period: str = Field(default="monthly", description="Time period for advice: daily, weekly, monthly")
...
class LangGraphAIAgent:
    ...
    def _extract_expense(self, state: AgentState) -> Dict:
        ...
        if message_type == "text":
            extracted_expense = self.expense_service.extract_expense_from_text(user_message)
        elif message_type == "image":
            # Assuming user_message contains image path for this implementation
            extracted_expense = self.expense_service.extract_expense_from_image(user_message)
    ...
    def _route_after_extraction(self, state: AgentState) -> str:
        ...
        if extracted_expense and self.expense_service.validate_extracted_expense(extracted_expense):
    ...
    def run(self, user_message: str, user_id: str, session_id: str, 
        ...
        final_message = result["messages"][-1].content if result["messages"] else "I'm sorry, I couldn't process that."
